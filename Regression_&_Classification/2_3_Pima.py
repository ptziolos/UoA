...
def main(file):
    ...
    for i in range(5):
        ...
        prob_a_accuracy += calculate_prob_a(mean_value_0, mean_value_1, sets[0][1])
        prob_b_accuracy += calculate_prob_b(mean_value_0, covariance_matrix_0, mean_value_1,
                                            covariance_matrix_1, sets[0][1])
        prob_c_accuracy += calculate_prob_c(mean_value_0, variance_0, mean_value_1, variance_1, sets[0][1])
        prob_d_accuracy += calculate_prob_d(variance_0, variance_1, len(data), classes[0], classes[1], sets[0][1])
...
def calculate_prob_a(mean_value_0, mean_value_1, test_set):
    # Euclidean Distance
    ...
    for query_example in test_set:
        ...
        if d0 < d1:
            if query_example[len(query_example) - 1] == "1":
                misclassifications += 1
        elif d0 > d1:
            if query_example[len(query_example) - 1] == "0":
                misclassifications += 1
    return 100-(misclassifications/len(test_set))*100


def calculate_prob_b(mean_value_0, covariance_matrix_0, mean_value_1, covariance_matrix_1, test_set):
...
def calculate_prob_c(mean_value_0, variance_0, mean_value_1, variance_1, test_set):
...
def calculate_prob_d(variance_0, variance_1, n, class_0, class_1, test_set):
    # Naive Bayes for Parzen windows with Gaussian kernels
    # for the optimal h = (1.06 * n^(-1/5) * sigma) we get better results
    misclassifications = 0
    for query_example in test_set:
        p0 = 1
        p1 = 1
        ...
        for index in range(len(query_example) - 1):
            ...
            for example in class_0:
                k0 += parzen_kernel(h0square, float(query_example[index]), float(example[index]))
            p0 = p0 * k0 * numpy.power(h0square, 3/2)
            for example in class_1:
                k1 += parzen_kernel(h1square, float(query_example[index]), float(example[index]))
            p1 = p1 * k1 * numpy.power(h1square, 3/2)
