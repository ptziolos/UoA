...
public class PotionMenu extends JPanel {
	...
	private MyFrame fr;
	private CompletableFuture<AbstractPotion> f1;
	private JPanel p = this;
	private List<AbstractPotion> potions = new ArrayList<AbstractPotion>();
	private ArrayList<MyLabel> l = new ArrayList<MyLabel>();
	private MyLabel selectedName;
	private AbstractPotion selectedPotion;
	...
	public void createMenu(List<AbstractPotion> potion) {
		this.potions.clear();
		this.potions.addAll(potion);
		
		l.clear();
		p.removeAll();
		
		for (int i = 0; i < potions.size(); i ++) {
			n1 = new MyLabel(potions.get(i).getName() + " (" + Integer.toString(potions.get(i).usesLeft()) + ")");
			l.add(n1);
			p.add(n1);
		}
		
		selectedName = l.get(index);
		selectedPotion = this.potions.get(index);
		...
		public void keyPressed(KeyEvent e) {
			if (e.getKeyCode() == KeyEvent.VK_DOWN) {
				if (index < l.size()-1) {
					index += 1;
					selectedName = l.get(index);
					selectedPotion = potions.get(index);
					i2 += 40;
					repaint();
				}
			}
	...
	public void pass(CompletableFuture<AbstractPotion> f1, MyFrame fr) {
		this.f1 = f1;
		this.fr = fr;
	}
}
