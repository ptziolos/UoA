...
optimizers_dictionary = {
	"o1": o.Adam(learning_rate=lr),
	"o2": o.Adamax(learning_rate=lr),
	"o3": o.Nadam(learning_rate=lr),
	"o4": o.RMSprop(learning_rate=lr),
	"o5": op.AdamW(learning_rate=lr,weight_decay=1e-5),
	"o6": op.LAMB(learning_rate=lr),
	"o7": op.LazyAdam(learning_rate=lr),
	"o8": op.RectifiedAdam(learning_rate=lr),
	"o9": op.Yogi(learning_rate=lr),
}
...
# Dice loss function
	...
	def dice(ytrue , ypred, smooth=1e-5): 
		true_positive = tf.reduce_sum(ytrue*ypred)
		predicted_positive = tf.reduce_sum(ypred * ytrue) + tf.reduce_sum(ypred * (1-ytrue))
		actual_positive = tf.reduce_sum(ytrue * ypred) + tf.reduce_sum(ytrue * (1-ypred))
		precision = true_positive/(predicted_positive)
		recall = true_positive/(actual_positive)
		numerator = 2 * precision * recall
		denominator = precision + recall
		f1 = (numerator + smooth) / (denominator + smooth)
		return 1 - f1
...
# Train the model
	...
	checkpointer = tf.keras.callbacks.ModelCheckpoint(
	                                   filepath = r"/home/ptziolos/Desktop/top_optimizer_results/" + net.name+"_"+codename+"_"+dataset, 
...
# Save history
	...
	with open(r"/home/ptziolos/Desktop/top_optimizer_results/" + net.name+"_"+codename+"_"+dataset + "__History", 'wb') as the_file:
