...
def create_model(img_size, weights, state):
	ResNet50_base = ResNet50(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))

	ResNet50_base.trainable = state
	...
	c1 = m.Model(inputs=ResNet50_base.input, outputs=ResNet50_base.get_layer('conv1_relu').output)
	c2 = m.Model(inputs=ResNet50_base.input, outputs=ResNet50_base.get_layer('conv2_block1_1_relu').output)
	c3 = m.Model(inputs=ResNet50_base.input, outputs=ResNet50_base.get_layer('conv3_block1_1_relu').output)
	c4 = m.Model(inputs=ResNet50_base.input, outputs=ResNet50_base.get_layer('conv4_block1_1_relu').output)

	# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = ResNet50_base(inputs)
