...
def create_model(img_size, weights, state):
	ResNet101V2_base = ResNet101V2(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	ResNet101V2_base.trainable = state
	
	"""
	# make last layers trainable
	for i,layer in enumerate(ResNet101V2_base.layers):
		if i < 366:
	...
	c1 = m.Model(inputs=ResNet101V2_base.input, outputs=ResNet101V2_base.get_layer('conv1_conv').output)
	c2 = m.Model(inputs=ResNet101V2_base.input, outputs=ResNet101V2_base.get_layer('conv2_block1_preact_relu').output)
	c3 = m.Model(inputs=ResNet101V2_base.input, outputs=ResNet101V2_base.get_layer('conv2_block3_2_relu').output)
	c4 = m.Model(inputs=ResNet101V2_base.input, outputs=ResNet101V2_base.get_layer('conv3_block4_2_relu').output)



	# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = ResNet101V2_base(inputs)
