...
def create_model(img_size, weights, state):
	DenseNet169_base = DenseNet169(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	DenseNet169_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(DenseNet169_base.layers):
		if i < 567:
	...
	c1 = m.Model(inputs=DenseNet169_base.input, outputs=DenseNet169_base.get_layer('conv1/conv').output)
	c2 = m.Model(inputs=DenseNet169_base.input, outputs=DenseNet169_base.get_layer('conv2_block1_0_relu').output)
	c3 = m.Model(inputs=DenseNet169_base.input, outputs=DenseNet169_base.get_layer('conv3_block1_0_relu').output)
	c4 = m.Model(inputs=DenseNet169_base.input, outputs=DenseNet169_base.get_layer('conv4_block1_0_relu').output)
	
# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = DenseNet169_base(inputs)
