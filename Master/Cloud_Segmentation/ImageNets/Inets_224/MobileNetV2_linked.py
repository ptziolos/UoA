...
def create_model(img_size, weights, state):
	MobileNetV2_base = MobileNetV2(weights=weights,
			include_top=False,
	...
	c1 = m.Model(inputs=MobileNetV2_base.input, outputs=MobileNetV2_base.get_layer('block_1_expand_relu').output)
	c2 = m.Model(inputs=MobileNetV2_base.input, outputs=MobileNetV2_base.get_layer('block_3_expand_relu').output)
	c3 = m.Model(inputs=MobileNetV2_base.input, outputs=MobileNetV2_base.get_layer('block_6_expand_relu').output)
	c4 = m.Model(inputs=MobileNetV2_base.input, outputs=MobileNetV2_base.get_layer('block_13_expand_relu').output)
