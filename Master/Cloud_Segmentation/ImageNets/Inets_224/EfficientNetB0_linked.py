...
def create_model(img_size, weights, state):
	EfficientNetB0_base = EfficientNetB0(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	EfficientNetB0_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(EfficientNetB0_base.layers):
		if i < 221:
	...
	c1 = m.Model(inputs=EfficientNetB0_base.input, outputs=EfficientNetB0_base.get_layer('block2a_expand_activation').output)
	c2 = m.Model(inputs=EfficientNetB0_base.input, outputs=EfficientNetB0_base.get_layer('block3a_expand_activation').output)
	c3 = m.Model(inputs=EfficientNetB0_base.input, outputs=EfficientNetB0_base.get_layer('block4a_expand_activation').output)
	c4 = m.Model(inputs=EfficientNetB0_base.input, outputs=EfficientNetB0_base.get_layer('block6a_expand_activation').output)
	
# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = EfficientNetB0_base(inputs)
