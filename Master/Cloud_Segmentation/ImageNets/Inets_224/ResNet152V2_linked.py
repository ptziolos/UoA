...
def create_model(img_size, weights, state):
	ResNet152V2_base = ResNet152V2(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	ResNet152V2_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(ResNet152V2_base.layers):
		if i < 553:
	...
	c1 = m.Model(inputs=ResNet152V2_base.input, outputs=ResNet152V2_base.get_layer('conv1_conv').output)
	c2 = m.Model(inputs=ResNet152V2_base.input, outputs=ResNet152V2_base.get_layer('conv2_block1_preact_relu').output)
	c3 = m.Model(inputs=ResNet152V2_base.input, outputs=ResNet152V2_base.get_layer('conv2_block3_2_relu').output)
	c4 = m.Model(inputs=ResNet152V2_base.input, outputs=ResNet152V2_base.get_layer('conv3_block8_2_relu').output)


	# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = ResNet152V2_base(inputs)
