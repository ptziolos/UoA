...
def create_model(img_size, weights, state):
	EfficientNetB7_base = EfficientNetB7(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	EfficientNetB7_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(EfficientNetB7_base.layers):
		if i < 795:
	...
	c1 = m.Model(inputs=EfficientNetB7_base.input, outputs=EfficientNetB7_base.get_layer('block2a_expand_activation').output)
	c2 = m.Model(inputs=EfficientNetB7_base.input, outputs=EfficientNetB7_base.get_layer('block3a_expand_activation').output)
	c3 = m.Model(inputs=EfficientNetB7_base.input, outputs=EfficientNetB7_base.get_layer('block4a_expand_activation').output)
	c4 = m.Model(inputs=EfficientNetB7_base.input, outputs=EfficientNetB7_base.get_layer('block6a_expand_activation').output)
	
# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = EfficientNetB7_base(inputs)
