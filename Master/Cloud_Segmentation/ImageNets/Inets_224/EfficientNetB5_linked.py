...
def create_model(img_size, weights, state):
	EfficientNetB5_base = EfficientNetB5(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	EfficientNetB5_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(EfficientNetB5_base.layers):
		if i < 558:
	...
	c1 = m.Model(inputs=EfficientNetB5_base.input, outputs=EfficientNetB5_base.get_layer('block2a_expand_activation').output)
	c2 = m.Model(inputs=EfficientNetB5_base.input, outputs=EfficientNetB5_base.get_layer('block3a_expand_activation').output)
	c3 = m.Model(inputs=EfficientNetB5_base.input, outputs=EfficientNetB5_base.get_layer('block4a_expand_activation').output)
	c4 = m.Model(inputs=EfficientNetB5_base.input, outputs=EfficientNetB5_base.get_layer('block6a_expand_activation').output)
	
# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = EfficientNetB5_base(inputs)
