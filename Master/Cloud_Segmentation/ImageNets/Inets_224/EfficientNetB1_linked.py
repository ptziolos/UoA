...
def create_model(img_size, weights, state):
	EfficientNetB1_base = EfficientNetB1(weights=weights,
			include_top=False,
	...
	c1 = m.Model(inputs=EfficientNetB1_base.input, outputs=EfficientNetB1_base.get_layer('block2a_expand_activation').output)
	c2 = m.Model(inputs=EfficientNetB1_base.input, outputs=EfficientNetB1_base.get_layer('block3a_expand_activation').output)
	c3 = m.Model(inputs=EfficientNetB1_base.input, outputs=EfficientNetB1_base.get_layer('block4a_expand_activation').output)
	c4 = m.Model(inputs=EfficientNetB1_base.input, outputs=EfficientNetB1_base.get_layer('block6a_expand_activation').output)
