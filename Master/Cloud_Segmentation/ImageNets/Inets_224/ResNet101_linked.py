...
def create_model(img_size, weights, state):
	ResNet101_base = ResNet101(weights=weights,
			include_top=False,
			input_shape=img_size+(3,))
	
	ResNet101_base.trainable = state

	"""
	# make last layers trainable
	for i,layer in enumerate(ResNet101_base.layers):
		if i < 335:
	...
	c1 = m.Model(inputs=ResNet101_base.input, outputs=ResNet101_base.get_layer('conv1_relu').output)
	c2 = m.Model(inputs=ResNet101_base.input, outputs=ResNet101_base.get_layer('conv2_block1_1_relu').output)
	c3 = m.Model(inputs=ResNet101_base.input, outputs=ResNet101_base.get_layer('conv3_block1_1_relu').output)
	c4 = m.Model(inputs=ResNet101_base.input, outputs=ResNet101_base.get_layer('conv4_block1_1_relu').output)


	# Network
	inputs = tf.keras.Input(shape=(img_size+(3,)))
	out_1 = ResNet101_base(inputs)
